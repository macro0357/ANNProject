...
class HNSW(object):
    ...
    def __init__(self, distance_type, m=5, ef=200, m0=None, heuristic=True, vectorized=False):
        ...
        if vectorized:
            ...
            # 返回list： query与其他vector的距离

        self._m = m # maximum number of connections for each element per layer
        self._ef = ef # size of the dynamic candidate list
        self._m0 = 2 * m if m0 is None else m0 # 第0层maximum number of connections
        self._level_mult = 1 / log2(m) # 减少层之间的重叠参数
        self._graphs = []
        self._enter_point = None

        self._select = ( # 启发式 选邻居
    ...
    def add(self, elem, ef=None):
        ...
        data = self.data # data[idx]表示第idx个点的vector，每次插入一个
        graphs = self._graphs
        ...
        if point is not None:  # the HNSW is not empty, we have an entry point
            ...
            for layer in reversed(graphs[level:]): # 从顶向下遍历graph，到（level+1）层停
                point, dist = self._search_graph_ef1(elem, point, dist, layer)# 返回layer里距离elem最近的点point 和距离dist
            # at these levels we have to insert elem; ep is a heap of entry points.
            ep = [(-dist, point)] # 从level层开始插入elem。
            layer0 = graphs[0]
            for layer in reversed(graphs[:level]): # 从level层向下遍历
                level_m = m if layer is not layer0 else self._m0 # level_m：level层的最大连接数for each element
                # navigate the graph and update ep with the closest
                # nodes we find
                ep = self._search_graph(elem, ep, layer, ef) # 返回距离elem最近的ef个点
                # insert in g[idx] the best neighbors
                layer[idx] = layer_idx = {} # 第idx个点在layer层的邻居
                
                self._select(layer_idx, ep, level_m, layer, heap=True) # 搜索邻居
